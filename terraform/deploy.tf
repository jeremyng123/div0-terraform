###############################################################################
# Render secrets + bootstrap locally (gitignored), push everything into the
# container, and bring the stacks up.
###############################################################################
...
resource "terraform_data" "deploy_stacks" {
  ...
  # container is recreated, a compose file or secret changes, the bootstrap
  # changes, or deploy_version is bumped.
  triggers_replace = {
    container_id   = proxmox_virtual_environment_container.docker.id
